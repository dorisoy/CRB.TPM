...
public abstract class ExcelProviderAbstract : IExcelProvider
{
    ...
    public async Task<IResultModel<ExcelModel>> Export<T>(ExcelExportEntityModel<T> model) where T : IEntity
    {
        ...
        if (string.IsNullOrEmpty(model.Title))
            throw new NullReferenceException("文件标题未定义");
        ...
        //创建文件
         var fs = new FileStream(excelModel.StoragePath, FileMode.Create, FileAccess.Write);
    ...
    public IResultModel<ExcelModel> ExportByTemplate(string templatePath, IList<Dictionary<string, object>> properties)
    {
