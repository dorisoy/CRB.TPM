...
public class JwtCredentialBuilder : ICredentialBuilder
{
    ...
    public async Task<ICredential> Build(List<Claim> claims)
    {
        ...
        {
            AccountId = Guid.Parse(claims.First(m => m.Type == CRBTPMClaimTypes.ACCOUNT_ID).Value),
            LoginTime = claims.First(m => m.Type == CRBTPMClaimTypes.LOGIN_TIME).Value.ToLong(),
            AccessToken = token,
