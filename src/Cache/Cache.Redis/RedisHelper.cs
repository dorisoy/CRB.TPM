...
public class RedisHelper : IDisposable
{
    ...
    /// <returns></returns>
    public async Task<bool> StringBatchSetAsync<T>(IList<(string key, T obj, TimeSpan? expiry)> setDatas)
    {
        var batch = Db.CreateBatch();
        List<Task<bool>> tasks = new List<Task<bool>>();
        foreach (var data in setDatas)
        {
            tasks.Add(Database.StringSetAsync(data.key, data.obj, data.expiry));
        }
        batch.Execute();
        var resList = await Task.WhenAll(tasks.ToArray());
        return !resList.Any(a => !a);
    }
    ...
    /// <param name="setDatas"></param>
    /// <returns></returns>
    public bool StringBatchSetTransaction<T>(IList<(string key, T obj, TimeSpan? expiry)> setDatas)
    {
        var tran = Db.CreateTransaction();
        List<Task<bool>> tasks = new List<Task<bool>>();
        foreach (var data in setDatas)
        {
            string val = Db.StringGet(data.key);
            tran.AddCondition(Condition.StringEqual(data.key, val));
            tran.StringSetAsync(data.key, _redisSerializer.Serialize(data.obj), data.expiry);
        }
    ...
    /// <returns></returns>
    public async Task<IList<T>> StringBatchGetAsync<T>(IList<string> keyList)
    {
        var batch = Db.CreateBatch();
        List<Task<T>> tasks = new List<Task<T>>();
        foreach (var key in keyList)
        {
            tasks.Add(Database.StringGetAsync<T>(key));
        }
        batch.Execute();
        var resList = await Task.WhenAll(tasks.ToArray());
    ...
    /// <param name="keyList"></param>
    /// <returns></returns>
    public async Task<IList<bool>> KeyExistsAllReturnAsync(IList<string> keyList)
    {
        var batch = Db.CreateBatch();
        List<Task<bool>> tasks = new List<Task<bool>>();
        foreach (var key in keyList)
        {
            tasks.Add(batch.KeyExistsAsync(key));
        }
        batch.Execute();
        var resList = await Task.WhenAll(tasks.ToArray());
        return resList;
    ...
    {
        var resList = await KeyExistsAllReturnAsync(keyList);
        return resList.AsParallel().All(a => a);
    }
    /// <summary>
    /// 是否包含所有键
    /// </summary>
