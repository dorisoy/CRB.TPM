...
using CRB.TPM.Utils.ClayObject.Extensions;
...
public abstract partial class RepositoryAbstract<TEntity> : IRepository<TEntity> where TEntity : IEntity, new()
{
    ...
    {
        var conn = OpenConn(uow, out IDbTransaction tran, NodeType.Master);
    ...
    /// <param name="commandType"></param>
    /// <returns></returns>
    public IEnumerable<ExpandoObject> QueryExpando(string sql, object param = null, IUnitOfWork uow = null, CommandType? commandType = null)
    {
        var conn = OpenConn(uow, out IDbTransaction tran, NodeType.Slave);
        return  conn.Query<ExpandoObject>(sql, param, tran, commandType: commandType).Select(x => x.ToExpando());
    }
    ...
    public string GetShardingTableName(DateTime date, IEntity entity = null)
    {

        if (EntityDescriptor.IsSharding)
        {
            //分表下，有两种情况
            //写入时如果表不存在则根据实体来创建表结构
            //读取时如果表不存在则（提示异常）
            var provider = DbContext.CodeFirstProvider;
            var option = provider.GetOptions();
            if (option == null)
            {
                throw new ArgumentException("分表下，更新或者创建表结构时须配置CodeFirst 配置项目~");
            }

            if (!option.EnableShardingPolicy)
            {
                return EntityDescriptor.TableName;
            }

            //创建表结构时必须配置 CodeFirst 配置项目
            if (provider != null)
            {
    ...
    private IDbConnection OpenConn(IUnitOfWork uow, out IDbTransaction transaction, NodeType? nodeType = null)
    {
        //SQLite无法使用事务
        ...
        transaction = null;
        return DbContext.NewConnection(nodeType);
    }
