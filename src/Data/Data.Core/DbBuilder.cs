...
internal class DbBuilder : IDbBuilder
{
    ...
    private readonly Type _dbClientContextType;

    public IServiceCollection Services { get; set; }
    public DbOptions Options { get; set; }
    public CodeFirstOptions CodeFirstOptions { get; set; }
    public IDbContext DbContext { get; set; }
    public IClientDbContext ClientDbContext { get; set; }

    public DbBuilder(IServiceCollection services, DbOptions options, Type dbContextType, Type dbClientContextType = null)
    {
        Services = services;
        Options = options;
        _dbContextType = dbContextType;
        _dbClientContextType = dbClientContextType;
    }
    ...
    public void Build()
    {
        //不启用读写分离，且数据库不为Sqlite时，检查数据库连接
        if (Options.Provider != DbProvider.Sqlite && !Options.UseReadWriteSeparation)
    ...
    /// <summary>
    /// 添加SqlMapper -> TypeHandler
    /// </summary>
    /// <param name="dbProvider"></param>
    private void AddTypeHandler(DbProvider  dbProvider)
    {
        switch (dbProvider)
        {
    ...
    private void CreateDbContext()
    {
        ...
        var dTPger = new DTPger(Options, sp.GetService<IDTPgerProvider>());
        var accountResolver = sp.GetService<IOperatorResolver>();

        //添加SqlMapper -> TypeHandler
        AddTypeHandler(Options.Provider);
      
        //获取数据库适配器的程序集 eg. "CRB.TPM.Data.Adapter.MySql"
        var aname = Assembly.GetCallingAssembly().GetName().Name;
        var dbAdapterAssemblyName = aname!.Replace("Core", "Adapter.") + Options.Provider;
        ...
        _dbContextType.GetProperty("Logger")?.SetValue(DbContext, dTPger);
        ...
        //启用客户端
        if (Options.UseClientMode && _dbClientContextType != null)
        {
            ClientDbContext = (IClientDbContext)Activator.CreateInstance(_dbClientContextType);
            _dbClientContextType.GetProperty("Options")?.SetValue(ClientDbContext, Options);
            //添加数据库上下文（客户端单例模式）
            Services.AddSingleton(_dbClientContextType, ClientDbContext);
            Services.AddSingleton(typeof(IClientDbContext), ClientDbContext);
            ClientDbContext.Init();
        }
           
        //使用读写分离
        if (Options.UseReadWriteSeparation)
        {
            var readWriteConnectorFactory = sp.GetService<IReadWriteConnectorFactory>();
            var readWriteManager = sp.GetService<IReadWriteManager>();
            var connectionStringManager = new ReadWriteConnectionStringManager(Options, readWriteConnectorFactory);
            _dbContextType.GetProperty("ConnectionStringManager")?.SetValue(DbContext, connectionStringManager);
        }
    ...
    private void LoadRepositories()
    {
        ...
        foreach (var assembly in _repositoryAssemblies)
        {
            ...
            foreach (var type in repositoryTypes)
            {
                ...
                //如：interface IAccountRepository : IRepository<AccountEntity>
