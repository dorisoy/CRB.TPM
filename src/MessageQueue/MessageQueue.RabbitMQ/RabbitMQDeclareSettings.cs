using System.Collections.Generic;
using RabbitMQ.Client;

namespace CRB.TPM.MessageQueue.RabbitMQ;

/// <summary>
/// 队列定义设置
/// </summary>
public class RabbitMQDeclareSettings
{
    /// <summary>
    /// 交换器设置
    /// </summary>
    public ExchangeDeclareSettings Exchange { get; set; } = new ExchangeDeclareSettings();

    /// <summary>
    /// 队列设置
    /// </summary>
    public QueueDeclareSettings Queue { get; set; } = new QueueDeclareSettings();
}

/// <summary>
/// 队列定义设置
/// </summary>
public class BindQuerySettings
{
    public BindQuerySettings(ExchangeDeclareSettings exchange, QueueDeclareSettings queue)
    {
        this.Exchange = exchange;
        this.Queue = queue;
    }
    public BindQuerySettings(ExchangeDeclareSettings exchange, QueueDeclareSettings queue, IList<string> routingKeyList)
        : this(exchange, queue)
    {
        this.RoutingKeyList = routingKeyList;
    }
    public BindQuerySettings(ExchangeDeclareSettings exchange, QueueDeclareSettings queue, IDictionary<string, object> headers)
        : this(exchange, queue)
    {
        this.Headers = headers;
    }
    /// <summary>
    /// 交换器设置
    /// </summary>
    public ExchangeDeclareSettings Exchange { get; set; } = new ExchangeDeclareSettings();

    /// <summary>
    /// 队列设置
    /// </summary>
    public QueueDeclareSettings Queue { get; set; } = new QueueDeclareSettings();

    /// <summary>
    /// 路由集合
    /// </summary>
    public IList<string> RoutingKeyList { get; set; } = new List<string>();
    /// <summary>
    /// 交换器
    /// </summary>
    public IDictionary<string, object> Headers { get; set; } = new Dictionary<string, object>();
}

/// <summary>
/// 交换器定义设置
/// </summary>
public class ExchangeDeclareSettings
{
    /// <summary>
    /// 交换器名称
    /// </summary>
    public string Name { get; set; } = DefaultExchange.Direct;

    /// <summary>
    /// 交换器类型
    /// </summary>
    public string Type { get; set; } = ExchangeType.Direct;

    /// <summary>
    /// 是否持久化
    /// </summary>
    public bool Durable { get; set; } = true;

    /// <summary>
    /// 自动删除
    /// </summary>
    public bool AutoDelete { get; set; }

    /// <summary>
    /// 交换器
    /// </summary>
    public IDictionary<string, object> Arguments { get; set; }
}

/// <summary>
/// 队列定义设置
/// </summary>
public class QueueDeclareSettings
{
    /// <summary>
    /// 队列名称
    /// </summary>
    public string Name { get; set; }

    /// <summary>
    /// 是否持久化
    /// </summary>
    public bool Durable { get; set; } = true;

    /// <summary>
    /// 是否排他
    /// </summary>
    public bool Exclusive { get; set; }

    /// <summary>
    /// 自动删除
    /// </summary>
    public bool AutoDelete { get; set; }

    /// <summary>
    /// 交换器
    /// </summary>
    public IDictionary<string, object> Arguments { get; set; }
}