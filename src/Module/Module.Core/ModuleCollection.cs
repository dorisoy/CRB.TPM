...
public class ModuleCollection : CollectionAbstract<ModuleDescriptor>, IModuleCollection
{
    ...
    /// 从配置目录加载模块
    /// </summary>
    /// <param name="commonOptions">全局通用配置</param>
    public void Load(CommonOptions commonOptions)
    {
        ...
        foreach (var dir in moduleDirs)
        {
            ...
            var moduleOptions = _configuration.Get<ModuleDbOptions>($"CRB.TPM:Modules:{code}:Db");

            //如果模块读写分离未开启时，则使用公共配置
            if (!moduleOptions.UseReadWriteSeparation)
            {
            ...
            {
                options.Db = moduleOptions;
            }


            _configuration.GetSection($"CRB.TPM:Modules:{code}").Bind(options);
