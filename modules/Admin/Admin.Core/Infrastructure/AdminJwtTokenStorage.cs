...
using CRB.TPM.Mod.Admin.Core.Domain.JwtAuthInfo;
...
internal class AdminJwtTokenStorage : IJwtTokenStorage
{
    ...
    {
        var platform = claims.First(m => m.Type == CRBTPMClaimTypes.PLATFORM).Value.ToInt();
        ...
        entity.LoginIP = claims.First(m => m.Type == CRBTPMClaimTypes.LOGIN_IP).Value;
