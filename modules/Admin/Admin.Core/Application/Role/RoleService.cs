...
public class RoleService : IRoleService
{
    ...
    public async Task<IResultModel> UpdateBindMenus(RoleBindMenusUpdateDto dto)
    {
        ...
        //清除关联账户的权限缓存
        var accRoles = await _accountRoleRepository.QueryByRole(dto.RoleId);
        var accounts = accRoles.Select(s => s.AccountId).ToList();

        var accountIds = await _accountRepository.Find(m => accounts.Contains(m.Id)).Select(m => m.Id).ToList<Guid>();
        if (accountIds.Any())
        {
