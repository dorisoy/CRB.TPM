  ...
  <m-container>
    <m-flex-row>
      <m-flex-fixed width="300px" class="m-margin-r-10">
        <m-list-box ref="listBoxRef" :title="$t(`tpm.routes.${page.name}`)" :icon="page.icon"
          :action="$tpm.api.admin.dictGroup.query" @change="handleGroupChange">
          <template #toolbar>
          ...
          <template #action="{ item }">
            <m-button-edit text="" :code="page.buttons.groupEdit.code" @click.stop="edit(item)" @success="refresh">
            </m-button-edit>
            <m-button-delete text="" :code="page.buttons.groupRemove.code" :action="$tpm.api.admin.dictGroup.remove"
              :data="item.id" @success="refresh"></m-button-delete>
          </template>
        </m-list-box>
      </m-flex-fixed>
      <m-flex-auto>
