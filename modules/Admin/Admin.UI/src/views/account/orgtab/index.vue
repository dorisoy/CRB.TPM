<template>
  ...
  <el-row class="m-roleorg-tree2" :style="{ height: '270px' }">
    ...
    <el-col :span="4" class="m-roleorg-border m-roleorg-tag" @drop="drop($event, 6)" @dragover="allowDrop($event)">
      <el-tag v-if="stationsSelections.length > 0" v-for="tag in stationsSelections" :key="tag.id" class="mx-1" closable
        @close="handleClose(tag, stationsSelections, 6)">
        {{ tag.name }}
      </el-tag>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const headOfficeChange = async (data) => {
      loading2.value = true
      const mOrgs = props.role?.extends
      if (data != null) {
