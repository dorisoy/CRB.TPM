<template>
  <m-list ref="listRef" :title="$t(`tpm.routes.${page.name}`)" :icon="page.icon" :cols="cols" :query-model="model" :query-method="query" :query-on-created="false">
    <template #querybar>
    ...
    <template #expand="{ row }">
      <el-descriptions :column="4">
        ...
        <el-descriptions-item :label="$t('tpm.remarks')" :span="4">{{ row.remarks }}</el-descriptions-item>
      </el-descriptions>
    </template>
    ...
    <template #col-show="{ row }">
      <span>{{ $t(row.show ? 'tpm.yes' : 'tpm.no') }}</span>
    </template>
