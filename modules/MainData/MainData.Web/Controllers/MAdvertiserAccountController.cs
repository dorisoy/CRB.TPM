...
{
    [OpenApiTag("MAdvertiserAccount", AddToDocument = true, Description = "广告商银行账号")]
    public class MAdvertiserAccountController : Web.ModuleController
    {
        private readonly IMAdvertiserAccountService _service;

        public MAdvertiserAccountController(IMAdvertiserAccountService service)
        {
            _service = service;
        }
        ...
        [HttpGet]
        [Description("查询")]
        public async Task<IResultModel> Query([FromQuery]MAdvertiserAccountQueryDto dto)
        {
