...
namespace MessageQueue.RabbitMQ.Tests.UnitTests
{
    ...
    public class DirectTests
    {
        ...
        const string routingKey1 = "routingKey1";
        const string routingKey2 = "routingKey2";


        public DirectTests(RabbitMQClient rabbitMQ, ILogger<DirectTests> logger)
        {
            ...
            var queueZongBuBusiness_Direct = new QueueDeclareSettings()
            {
                ...
                AutoDelete = false,
                Arguments = null,
                Name = "queueZongBuBusiness_Direct"
            };
            var queueSiChuanBusiness_Direct = new QueueDeclareSettings()
            {
                Durable = false,
                Exclusive = false,
                AutoDelete = false,
                Arguments = null,
                Name = "queueSiChuanBusiness_Direct"
            };
            _rabbitMQ.QueueBind(_settingsBusinessZongBu = new BindQuerySettings(_exchange, queueZongBuBusiness_Direct, new List<string>() { routingKey1, routingKey2 }));
            _rabbitMQ.QueueBind(_settingsBusinessSiChuan = new BindQuerySettings(_exchange, queueSiChuanBusiness_Direct, new List<string>() { routingKey1 }));
        }

        [Fact, Order(1)]
        public void Producer_Direct()
        {
            int num = 100;
            ...
            foreach (var data in datas)
            {
                ...
                _rabbitMQ.Send(data, _exchange.Name, routingKey2);
